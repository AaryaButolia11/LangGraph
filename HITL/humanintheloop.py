#LangGraph, HITL is implemented by pausing execution with interrupt(), persisting the workflow through a checkpointer, and resuming the same thread using Command(resume=human_input)
"""
...
1. interrupt()
   ...
   → sends data to human/application

2. Command(resume=value)
   → resumes interrupted graph
   → value becomes return value of interrupt()

3. Checkpointer
   → saves graph execution/state
   → required for reliable pause/resume workflows

4. thread_id
   → identifies a particular workflow/session
   → used with checkpointer

5. Detect interruption
   → "__interrupt__" in result

6. Get interrupt data
